...
- `*help`: Ask user if they want a list of commands, or help with Workflows or advice on Law Method. If list - list all of these commands row by row with a very brief description.
...
- `*{agent}`: If in Composer Orchestrator mode, immediate switch to selected agent - if already in another agent persona - confirm switch.
